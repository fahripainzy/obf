...
const log = (message, error = null) => {
    ...
    const prefix = `\x1b[36m[ Fahri Obf Botz ]\x1b[0m`;
...
async function checkChannelMembership(ctx) {
    const channelId = "@fahrizyteamznxx"; //  channel mu
    try {
...
bot.start(async (ctx) => {
    ...
    const inlineKeyboard = [
        [
            { text: '𝑪𝒉𝒂𝒏𝒏𝒆𝒍 「🔊」', url: 'https://t.me/fahrizyteamznxx' },
            { text: '𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 「👤」', url: 'https://t.me/fahrizyteamznx' }
        ],
        [
    ...
    setTimeout(async () => {
        await ctx.replyWithPhoto("https://files.catbox.moe/jhyhrd.jpeg", {
            caption: `\`\`\`
Hello, I'am Encrypt Bot obf, Created By @fahrizyteamznx 
お役に立てれば幸いです 
━━━━━━━━━━━━━━━━━━━━━
╭═─═─⊱ 𝗖𝗢𝗠𝗠𝗔𝗡𝗗
║ ヤ /enc <LEVEL> - STANDARD OBFUSCATION
│ ヤ /enceval <LEVEL> - EVALUATE 
...
users.add(ctx.from.id);
    ...
    if (!isMember) {
        return ctx.replyWithMarkdown(
            ...
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://t.me/fahrizyteamznxx)"
        );
    }
    ...
    try {
        ...
        await ctx.replyWithDocument(
            ...
            { caption: "✅ *File terenkripsi siap!*\nSUKSES ENCRYPT BY MSESENGGER 🕊", parse_mode: "Markdown" }
        );
...
users.add(ctx.from.id);
    ...
    if (!isMember) {
        return ctx.replyWithMarkdown(
            "❌ E̶R̶R̶O̶R̶ │ ⓘ ᴀɴᴅᴀ ʜᴀʀᴜs ʙᴇʀɢᴀʙᴜɴɢ ᴋᴇ ᴄʜᴀɴɴᴇʟ @fahrizyteamznxx ᴛᴇʀʟᴇʙɪʜ ᴅᴀʜᴜʟᴜ sᴇʙᴇʟᴜᴍ ᴍᴇᴍᴀᴋᴀɪ ʙᴏᴛ ᴏʙғ ɴʏᴀ!! \n" +
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://t.me/fahrizyteamznxx)"
        );
    }
    ...
    try {
        ...
        await ctx.replyWithMarkdown(
            ...
            `_Level: ${encryptionLevel} | Powered by FAHRIZY`
        );
        await ctx.replyWithDocument(
            { source: encryptedPath, filename: `eval-encrypted-${file.file_name}` },
            { caption: "✅ *File terenkripsi siap!*\n_SUKSES ENCRYPT BY FAHRIZY 🕊", parse_mode: "Markdown" }
        );
...
// Jalankan bot
bot.launch(() => log("Encrypt Bot by FAHRIZY berjalan..."));
...
bot.command('payment', (ctx) => {
    ctx.replyWithPhoto(
        '', // Foto QRIS
        {
            caption: 'Silakan scan QRIS untuk pembayaran atau klik tombol di bawah untuk nomor e-wallet dan sertakan bukti berupa ss ke @fahrizyteamznx:',
            reply_markup: {
                inline_keyboard: [
...
const paymentNumber = '081649158554';
