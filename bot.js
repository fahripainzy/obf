...
async function checkChannelMembership(ctx) {
    const channelId = "@fahrizyteamznxx"; // ID channel
    try {
...
bot.start(async (ctx) => {
    ...
    setTimeout(async () => {
        await ctx.replyWithPhoto("https://files.catbox.moe/xr0vnu.jpeg", {
            caption: `
...
║ ☍ ᴄʜᴀɴɴᴇʟ : @fahrizyteamznxx
┃ ☍ ᴅᴇᴠᴇʟᴏᴘᴇʀ : @fahrizyteamznx
╚━═━═━═━═━═━═━❍
Stay secure with FAHRI! 
    `, {
        parse_mode: "Markdown",
        reply_markup: {
            inline_keyboard: [
...
users.add(ctx.from.id);
    ...
    if (!isMember) {
        return ctx.replyWithMarkdown(
            ...
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://https://t.me/fahrizyteamznxx)"
        );
    }
    ...
    try {
        ...
        await ctx.replyWithDocument(
            ...
            { caption: "✅ *File terenkripsi siap!*\nSUKSES ENCRYPT BY FAHRI 🕊", parse_mode: "Markdown" }
        );
...
users.add(ctx.from.id);
    ...
    try {
        ...
        await ctx.replyWithMarkdown(
            ...
            `_Level: ${encryptionLevel} | Powered by FAHRI`
        );
        await ctx.replyWithDocument(
            { source: encryptedPath, filename: `eval-encrypted-${file.file_name}` },
            { caption: "✅ *File terenkripsi siap!*\n_SUKSES ENCRYPT BY FAHRI 🕊", parse_mode: "Markdown" }
        );
...
// Jalankan bot
bot.launch(() => log("Encrypt Bot by FAHRI berjalan..."));
...
const paymentNumber = '0882010414726';
